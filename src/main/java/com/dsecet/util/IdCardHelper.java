...
public class IdCardHelper{
    ...
    public String getBirthByIdCard(String idCard){
        if(!this.is18Idcard(idCard)){
            return null;
        }
        LocalDate localDate = LocalDate.parse(
                idCard.substring(6, 14), DateTimeFormatter.ofPattern("yyyyMMdd", Locale.CHINA));
        return localDate.format(DateTimeFormatter.ofPattern("yyyy/MM/dd", Locale.CHINA));
    }
     ...
     *
     * @param idCard 身份编号
     * @return M-男, F-女, N-不确定
     */
    public String getGenderByIdCard(String idCard){
        if(!this.is18Idcard(idCard)){
            return "N";
        }
        Integer genderNum = Integer.parseInt(idCard.substring(16, 17));
