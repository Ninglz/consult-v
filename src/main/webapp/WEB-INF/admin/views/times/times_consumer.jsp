...
<body>
<input type="hidden" id="activeMenu" value="timesConsumer">
                        ...
                        <form id="times_form" action="" method="post">
                           ...
                           <c:forEach var = "obj" items ="${list }" varStatus ="status" >
				               <c:if test="${status.count eq 1 || (status.count-1) % 7 eq 0}">    
				 				 ...
				 				 <td><input type="text" name="times" id="" class="fany_input w100" maxlength="4" value="${obj.consumerProportion}"/></td>
...
$(function(){
  ...
  function timesSubmit(){
      ...
      $.ajax({
          type: "POST",
          url: "/admin/times/save/consumer",
          data: params,
          success: function (data) {
        	  if(data="success"){
        		  $.dialog({type:2,text:"咨询者时间比例设置成功"});
          	 }else{
        		  $.dialog({type:2,text:"咨询者时间比例设置失败"});
          	 }
          }
      });
  }
